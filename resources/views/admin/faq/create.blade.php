...
@section('header-title', 'Add New FAQ')
@section('header-description', 'Create a frequently asked question')
        ...
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
            <div class="flex items-center">
                <div class="w-10 h-10 bg-[#2D6A4F]/10 rounded-xl flex items-center justify-center mr-3">
                    <i class="fas fa-question-circle text-[#2D6A4F] text-lg"></i>
                </div>
                <div>
                    <h3 class="text-lg font-semibold text-gray-800">FAQ Information</h3>
                    <p class="text-sm text-gray-500 mt-0.5">Enter the question and answer</p>
                </div>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('admin.faq.store') }}" class="p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="category" id="category" required
                        ...
                        <option value="cancellation" {{ old('category') == 'cancellation' ? 'selected' : '' }}>Cancellation</option>
                        <option value="property" {{ old('category') == 'property' ? 'selected' : '' }}>Property</option>
                ...
                <div>
                    ...
                    <input type="number" name="sort_id" id="sort_id" value="{{ old('sort_id', 0) }}"
                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#2D6A4F]">
            ...
            <div>
                ...
                <textarea name="answer" id="answer" rows="6" required
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#2D6A4F]"
                          placeholder="Provide a detailed answer to the question...">{{ old('answer') }}</textarea>
                @error('answer')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="flex items-center space-x-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="radio" name="is_active" value="1" {{ old('is_active', '1') == '1' ? 'checked' : '' }} class="w-4 h-4 text-[#2D6A4F]">
                        <span class="ml-2 text-sm text-gray-700">Active (visible to users)</span>
                    </label>
                    <label class="flex items-center cursor-pointer">
                        <input type="radio" name="is_active" value="0" {{ old('is_active') == '0' ? 'checked' : '' }} class="w-4 h-4 text-gray-400">
